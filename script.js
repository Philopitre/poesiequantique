...
// Initialisation de la liste de mots sélectionnés avec tous les mots au départ
selectedWords = [...words];
...
window.onload = function() {
  ...
  // Ajout du compteur de mots sélectionnés
...
// Ajouter un compteur de mots sélectionnés
...
// Fonction modifiée pour générer avec tous les mots disponibles
...
// Fonction modifiée pour générer avec tous les mots sélectionnés
...
// Fonction pour gérer le basculement entre mode clair et mode sombre
function toggleDarkMode() {
  ...
  // Jouer un effet sonore lors du changement
  const sound = document.getElementById('typewriterSound');
  if (sound && typeof sound.play === 'function') {
    sound.currentTime = 0;
    sound.play().catch(err => console.log('Audio playback error:', err));
  ...
  showNotification(isDarkMode ? "Mode sombre activé 🌙" : "Mode clair activé ☀️");
}

// Fonction pour initialiser le mode sombre en fonction des préférences enregistrées
function initDarkMode() {
  // Vérifier s'il existe une préférence enregistrée
  const savedDarkMode = localStorage.getItem('darkMode');
  
  // Vérifier si l'utilisateur a une préférence de système pour le mode sombre
  const prefersDarkMode = window.matchMedia && window.matchMedia('(prefers-color-scheme: dark)').matches;
  
  // Appliquer le mode sombre si enregistré comme 'true' ou si préférence système
  if (savedDarkMode === 'true' || (savedDarkMode === null && prefersDarkMode)) {
    document.body.classList.add('dark-mode');
    
    // Mettre à jour l'état du commutateur
    const toggleInput = document.getElementById('theme-switch');
    if (toggleInput) {
...
// Ajouter ces fonctions à l'initialisation de la page
window.addEventListener('DOMContentLoaded', function() {
  // Créer le commutateur de thème s'il n'existe pas déjà
...
// Fonction pour créer dynamiquement le commutateur de thème
function createThemeToggle() {
  ...
  // Ajouter l'écouteur d'événement pour le commutateur
  const toggleInput = document.getElementById('theme-switch');
  if (toggleInput) {
...
window.onload = function() {
  // Exécuter la fonction onload originale si elle existe
  if (typeof originalOnload === 'function') {
    originalOnload();
  }
  
  // Initialiser le mode sombre (si ce n'est pas déjà fait)
  if (!document.querySelector('.theme-switch-wrapper')) {
    createThemeToggle();
    initDarkMode();
  }
...
// Chargement du thème préféré au démarrage
window.addEventListener('DOMContentLoaded', () => {
  ...
  // Toujours ajouter la classe pour activer la transition
  document.body.classList.add('theme-transition');
});
